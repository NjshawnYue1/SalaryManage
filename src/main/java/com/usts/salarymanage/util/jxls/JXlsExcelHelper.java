package com.usts.salarymanage.util.jxls;
...
public class JXlsExcelHelper {
    ...
    public static final String TEMPLATE_DATA_KEY = "list";

    public static final String TEMPLATE_ATTR_KEY = "attr";

    public static final String TEMPLATE_HEADER_KEY = "excelVO";
    ...
    private static JXlsExcelHelper jXlsExcelHelper;

    static {
        ReaderConfig.getInstance().setSkipErrors(true);
        ReaderConfig.getInstance().setUseDefaultValuesForPrimitiveTypes(false);
    ...
    private JXlsExcelHelper() {
    }

    public static JXlsExcelHelper getInstance() {
        if (jXlsExcelHelper == null) {
            jXlsExcelHelper = new JXlsExcelHelper();
        }
        return jXlsExcelHelper;
    }

    public void export(Collection collection, InputStream in, OutputStream out) {
        check(in, out);
        context.putVar(TEMPLATE_DATA_KEY, collection);
        exec(in, out);
    }

    public void export(Map<String, Object> map, InputStream in, OutputStream out) {
        check(in, out);
        context.putVar(TEMPLATE_DATA_KEY, map.get(TEMPLATE_DATA_KEY));
        context.putVar(TEMPLATE_ATTR_KEY, map.get(TEMPLATE_ATTR_KEY));
        exec(in, out);
    }

    public void export(InputStream in, OutputStream out, Map<String, Object> map) {
        check(in, out);
    ...
    private void exec(InputStream in, OutputStream out) {
        try {
            if (in.available() <= 0) {
                in.reset();
            }
    ...
    public void setRespHeader(String exportName, String suffix, HttpServletResponse out) {
        if (out == null) {
            return;
        }
        out.setContentType(CONTENT_TYPE);
        out.addHeader(
                HEADER_CONTENT_DISPOSITION_KEY,
                genExportName(exportName, suffix)
        );
    }

    private String genExportName(String exportName, String suffix) {
        try {
            return HEADER_CONTENT_DISPOSITION_VALUE + URLEncoder.encode(exportName, "UTF-8") + suffix;
    ...
    public XLSReadStatus importExcel(InputStream config, InputStream in, Map<String, Object> importData) {
        XLSReadStatus xlsReadStatus = new XLSReadStatus();
        check(config, in);
        try {
            // 如果重复读取.需要reset.不然会报`Premature end of file`错误
            if (config.available() <= 0) {
                config.reset();
            }
            XLSReader mainReader = ReaderBuilder.buildFromXML(config);
            xlsReadStatus = mainReader.read(in, importData);
        } catch (Exception e) {
            e.printStackTrace();
            xlsReadStatus.setStatusOK(false);
        } finally {
            close(in);
        }
        if (CollectionUtils.isNotEmpty(xlsReadStatus.getReadMessages())) {
            xlsReadStatus.setStatusOK(false);
        }
        return xlsReadStatus;
    }

    private void check(Closeable... closeables) {
        for (Closeable closeable : closeables) {
            if (closeable == null) {
                throw new RuntimeException("excel导入/导出错误,没有找到输入/输出源!!");
            }
        }
    }
    ...
    public String getAllErrorMsg(XLSReadStatus xlsReadStatus) {
        sb.delete(0, sb.length());
        if (xlsReadStatus != null && CollectionUtils.isNotEmpty(xlsReadStatus.getReadMessages())) {
            for (Object readMessage : xlsReadStatus.getReadMessages()) {
                if (readMessage instanceof XLSReadMessage) {
                    XLSReadMessage xlsReadMessage = (XLSReadMessage) readMessage;
                    sb.append(xlsReadMessage.getMessage()).append(",");
                }
            }
        }
    ...
    public void convertExcelToCSV(String strSource, String strDestination) throws Exception {
        toCSV.convertExcelToCSV(strSource, strDestination);
    }
    ...
    public static class ToCSV {
         ...
         */
        private void openWorkbook(File file) throws Exception {
            ...
            if (row != null) {
                ...
                for (int i = 0; i <= lastCellNum; i++) {
                    ...
                    if (cell == null) {
                        ...
                        if (cell.getCellType() != DataValidationConstraint.ValidationType.FORMULA) {
